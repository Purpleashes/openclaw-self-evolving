...
### 🧠 MEMORY.md - Your Long-Term Memory (Three-Layer System)

**Three-Layer Memory Format:**
- `[P0]` — Permanent memory (identity, core preferences, key relationships)
- `[P1][YYYY-MM-DD]` — 90-day TTL (active projects, schedules)
- `[P2][YYYY-MM-DD]` — 30-day TTL (temporary notes, debug info)
...
- P0: Who the user is, core relationships, permanent preferences
- P1: Active projects, current team members, upcoming schedules (≤90 days)
- P2: Temporary notes, one-time debug info, short-term reminders (≤30 days)

**Memory Maintenance:**
- `scripts/memory-janitor.py` auto-archives expired P1/P2 entries
- Run weekly: Archive P2 >30 days, P1 >90 days
- Extract lessons to `memory/lessons/*.jsonl`
- NEVER delete P0 entries without explicit user confirmation
...
3. Update `MEMORY.md` with distilled learnings using [P0]/[P1][date]/[P2][date] format
4. Run `scripts/memory-janitor.py` to archive expired entries
5. Extract structured lessons to `memory/lessons/*.jsonl`
